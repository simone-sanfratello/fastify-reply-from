...
// For some reason we can have URLs that have 'null' (as a string)
// we need to remove that.
function notNullOrigin ({ origin }) {
  if (origin === 'null') {
    return null
  }
...
function buildRequest (opts) {
  ...
  const baseUrl = opts.base && notNullOrigin(new URL(opts.base))
  ...
  function handleUndici (opts, done) {
    const req = {
      ...
      origin: baseUrl || notNullOrigin(opts.url)
    }
